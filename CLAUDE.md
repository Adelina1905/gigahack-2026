...
- HTTP contract: `POST /v1/chat {chatId, message, history[{role,content}]}` → `{mode, status, answer, citations[{title,url,exactQuote,documentId}], clarificationChoices}` (`mode` is `rag`, `llm` or `demo`); 503 `{detail}` when the LLM is unavailable; `GET /health` → `{mode, llmConfigured, ragAvailable}`.
- `CHAT_MODE=demo` (default `live`) serves deterministic, labelled replies from `demo_service.DemoChatService` with no OpenRouter key or Qdrant index; any other value fails startup.
...
./mvnw test                       # DB tests are skipped unless CHAT_TEST_JDBC_URL is set (see below)
...
- The UI uses the DB-backed endpoints: `/api/chats` (CRUD, listed by `updatedAt`), `/api/chats/{id}/responses` (POST asks the LLM; `PUT /{responseId}` retries or regenerates an answer in place) and `/api/documents`. Rows are scoped by the `smart_city_client_id` cookie (`AnonymousClientCookie`). One `responses` row is one turn (`prompt` + `text`), and citations are linked through `response_documents` and returned as `documents`.
- Durable generation lifecycle (contract in SETUP.md, "Durable chat API"): the prompt is committed first as a `PENDING` turn, then `ResponseService` calls the LLM outside any transaction (`TransactionTemplate`) and stores the answer, the raw `aiReply` and the citations, or marks the turn `FAILED` with an `errorCode`. Client `requestId` UUIDs make chat creation, sends and regenerations idempotent; regenerate sends `{requestId, expectedGenerationVersion}`. Conflicts are 409s (`REQUEST_CONFLICT`, `STALE_GENERATION`, `CHAT_BUSY`: one generation per chat). `GenerationRecovery` fails turns still pending after five minutes. `AiResponseProvider.historyFrom` turns the earlier stored turns into history, trimmed to `app.llm.max-history`. Regenerating uses only the turns before that response.
...
- Tests inject fakes (e.g. a `@Primary` `RecordingLlmGateway` bean). DB tests extend `IsolatedDatabaseTest`, commit data, and run only when `CHAT_TEST_JDBC_URL` points at a database named `*_test` or `*_integration` (`CHAT_TEST_DB_USERNAME`/`CHAT_TEST_DB_PASSWORD`); they never read `.env`. JUnit conditions aren't inherited, so each subclass carries its own `@EnabledIfEnvironmentVariable`.
...
```
npm install
npm run dev | lint | typecheck | build | test   # vitest + jsdom needs Node >= 22
```
- Single page: `App.jsx` → `pages/Playground.tsx`, which composes `components/Sidebar.tsx` (the chat list from `hooks/useChats.ts`), `components/ChatWindow.tsx` and `hooks/useChat.ts` (per-chat threads, optimistic send, retry, regenerate; polls every 2 s while a turn is `PENDING`). `hooks/chatState.ts` reconciles local and server turns by `requestId`, and never lets an older `generationVersion` replace a newer one. The open chat is in the URL (`?chat=<uuid>`, `hooks/useActiveChatId.ts`), and a chat is created on the first message.
- `src/api/client.ts` holds the fetch calls (base `VITE_API_BASE_URL`, default `/api` through the Vite proxy; `ApiError.status` is 0 on network failure). `api/types.ts` mirrors the Java DTOs, `api/mappers.ts` is the only place they are mapped to UI types, and `api/index.ts` switches to `api/mockClient.ts` when `VITE_USE_MOCK=true`.
- `components/AssistantMessage.tsx` renders answers as Markdown (`react-markdown` + `remark-gfm`, raw HTML and images off). The RAG `[S1]` markers are stripped, and the cited documents show as `CitationPills` plus the `SourceList`. Every model- or citation-supplied link goes through `sources/safeLink.ts` (HTTP(S) only).
...
- UI text is in Romanian (default), Russian and English: `i18n/messages.ts` holds one typed dictionary per locale, with Romanian as the shape the others must match. `useI18n()` returns `{ locale, setLocale, t }`. The first visit picks the locale from `navigator.languages` (the browser's Accept-Language list), falling back to `ro`; a pick in `LanguageSwitcher` is saved in localStorage. Hooks store `ErrorKey`s, not sentences. Chats are still created as `DEFAULT_CHAT_NAME` ("New chat"), because the backend renames chats with that exact name after their first prompt, so the name is translated only for display.
