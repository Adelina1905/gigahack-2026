...
DEFAULT_MANIFEST_DIRECTORY = DATA_V2_DIRECTORY / "01_manifests"
DEFAULT_SOURCE_DIRECTORY = DATA_V2_DIRECTORY / "01_sources"
...
def ingest_source(arguments: argparse.Namespace) -> tuple[dict[str, Any], Path, bool]:
    ...
    document_id = f"{safe_slug(title)}-{digest[:12]}"
    source_type = arguments.source_type or infer_source_type(source, arguments.source_url)
    media_type = mimetypes.guess_type(source.name)[0] or "application/octet-stream"

    source_directory = arguments.source_dir.expanduser().resolve() / document_id
    ...
    manifest_path = arguments.manifest_dir.expanduser().resolve() / f"{document_id}.json"
